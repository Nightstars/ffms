...
<input type="hidden" value="${account.id}" id="accountEditId">
    ...
    <td height="215" valign="top"><table width="98%"  border="0" cellspacing="0" cellpadding="0">
      ...
      <table width="98%"  border="0" cellpadding="0" cellspacing="0" class="text008">
        ...
        <tr align="center">
          ...
          <td width="88%" align="left">
          <select class="inp001" id="accountEditCItem">
            <c:forEach var="item" items="${item}">
              <option value="${item.id}" ${account.item.id==item.id?"selected":""}>${item.name}</option>
            </c:forEach>
          </select>
          </td>
        </tr>
        <tr align="center" class="bg04">
          ...
          <td align="left"><input name="textfield" type="date" class="inp001" value="<fmt:formatDate pattern="yyyy-MM-dd" value="${account.date}"></fmt:formatDate>" id="accountEditDate"></td>
        </tr>
        <tr align="center">
          <td height="35" align="right">收支家庭成员</td>
          <td align="left">
          <select class="inp001" id="accountEditUser">
            <c:forEach var="user" items="${user}">
              <option value="${user.id}" ${account.user.id==user.id?"selected":""}>${user.name}</option>
            </c:forEach>
          </select>
          </td>
        </tr>
        <tr align="center" class="bg04">
          ...
          <td align="left"><input name="textfield" type="text" class="inp001" value="${account.amount}" id="accountEditAmount"></td>
        </tr>
        <tr align="center">
          <td height="168" align="right" valign="top" ><div class="mar009">备注</div></td>
          <td align="left" valign="top"><textarea name="textarea" class="inp002" id="accountEditRemark">${account.remark}</textarea>
         </td>
        </tr>
      </table>
